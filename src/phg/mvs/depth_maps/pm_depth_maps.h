...
public:
    PMDepthMapsBuilder(const size_t& ncameras, const std::vector<cv::Mat>& cameras_imgs, const std::vector<cv::Mat>& cameras_imgs_grey, const std::vector<std::string>& cameras_labels, const std::vector<matrix34d>& cameras_P,
        ...
        : ncameras(ncameras)
        , cameras_imgs(cameras_imgs)
        , cameras_imgs_grey(cameras_imgs_grey)
    ...
    void buildDepthMap(unsigned int camera_key, cv::Mat& depth_map, cv::Mat& normal_map, cv::Mat& cost_map, float depth_min, float depth_max);
    ...
    static void buildGoodPoints(const cv::Mat& depth_map, const cv::Mat& normal_map, const cv::Mat& cost_map, const cv::Mat& img, const phg::Calibration& calibration, const matrix34d& PtoWorld, std::vector<cv::Vec3d>& points,
